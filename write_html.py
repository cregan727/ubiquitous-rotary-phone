...
Log_final_out_path = system_input[0].split(",")[0]
summary_csv_path = str(system_input[1].split(",")[0]) + "/Gene/Summary.csv"
...
#load inputs as dfs
Log_final_out = pd.read_csv(Log_final_out_path, delimiter="\t", header=None, index_col=0)
...
def fixtableforhtml(tablecolumn):
    """
    fix the formatting to make the decimals into percents and add comma seps to long numbers
    """
    new_table = []
...
def outformathtml(pandasdf):
    ...
    pandas_table = pandas_table.replace("""<thead>""", """<thead style="text-align: left; color: #094D92; font-size: 30px"> """)
    return(pandas_table)
...
sequencing_pd = sequencing_pd.append(Log_final_out.loc[['Number of reads unmapped: too short']])
#Reads with Valid Barcodes
sequencing_pd = sequencing_pd.append(Summary_csv.loc[["Reads With Valid Barcodes"]])
#Sequencing Saturation
sequencing_pd = sequencing_pd.append(Summary_csv.loc[["Sequencing Saturation"]])
#Q30 Bases in CB+UMI
sequencing_pd = sequencing_pd.append(Summary_csv.loc[["Q30 Bases in CB+UMI"]])
#Q30 Bases in RNA Read
sequencing_pd = sequencing_pd.append(Summary_csv.loc[["Q30 Bases in RNA read"]])
...
mapping_pd = mapping_pd.append(Summary_csv.loc[["Reads Mapped to Genome: Unique+Multiple"]])
#Reads Mapped Confidently to Genome
mapping_pd = mapping_pd.append(Summary_csv.loc[["Reads Mapped to Genome: Unique"]])
#Reads Mapped Confidently to Transcriptome
mapping_pd = mapping_pd.append(Summary_csv.loc[["Reads Mapped to Transcriptome: Unique Genes"]])
mapping_pd.index = ['Reads Mapped to Genome', 'Reads Mapped Confidently to Genome', 'Reads Mapped Confidently to Transcriptome']
...
cell_pd = cell_pd.append(Summary_csv.loc[['Estimated Number of Cells']])
#Fraction of Reads in Cells
cell_pd = cell_pd.append(Summary_csv.loc[['Fraction of Reads in Cells']])
#Mean Reads per Cell
cell_pd = cell_pd.append(Summary_csv.loc[['Mean Reads per Cell']])
#Median Genes per Cell
cell_pd = cell_pd.append(Summary_csv.loc[['Median Genes per Cell']])
#Total Genes Detected
cell_pd = cell_pd.append(Summary_csv.loc[['Total Genes Detected']])
#Median UMI per Cell
cell_pd = cell_pd.append(Summary_csv.loc[['Median UMI per Cell']])
...
Template = Template.replace("MEDIANGENESPC", Summary_csv.loc["Median Genes per Cell"][1].replace(".0", ""))
